...
func (j *JWTer) GenerateToken(ctx context.Context, u entity.User) ([]byte, error) {
	tok, err := jwt.NewBuilder().
		...
		Issuer(`github.com/MatsuoTakuro/go_todo_app`).
...
func (j *JWTer) GetToken(ctx context.Context, r *http.Request) (jwt.Token, error) {
	...
	if _, err := j.Store.Load(ctx, tok.JwtID()); err != nil {
		return nil, fmt.Errorf("GetToken: %q expired: %w", tok.JwtID(), err)
	}
	return tok, nil
}
...
func SetUserID(ctx context.Context, uid entity.UserID) context.Context {
	// set userid from request to ctx
